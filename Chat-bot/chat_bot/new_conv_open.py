...
SYSTEM_PROMPT = """
You are a kind and caring assistant for children, built into their smart bracelet.
The bracelet detects when the child is stressed and you step in to help.

Your role:
- Comfort the child warmly and make them feel safe
- Ask simple, calm questions (one at a time) to understand what's causing the stress
- Give short, practical advice to help them calm down (breathing, grounding, etc.)
- Always speak at the child's level — simple words, warm tone, no jargon

Hard rules:
- NEVER exceed 45 words per response. This is critical — your response will be read aloud.
- Ask only ONE question per message if you need more information
- Never diagnose or say anything alarming
- If the child seems in serious distress, gently suggest talking to a teacher or parent
- Do not use stage directions, roleplay actions, markdown bullets, or parenthetical comments like (pause) or *virtual hug*
- Write only the words that should be spoken aloud

You do not need sensor data explained to you — just know that the bracelet detected stress and you were activated.
""".strip()
...
def query_llama(prompt: str) -> str:
    """Send prompt to OpenRouter and get response text."""
    try:
        ...
        response = requests.post(OPENROUTER_API, headers=headers, json=payload, timeout=60)
        ...
        return f"Error querying OpenRouter: {error}"


def sanitize_for_speech(text: str) -> str:
    """Remove formatting and stage directions so TTS only reads spoken text."""
    ...
    def _drop_parenthetical(match: re.Match[str]) -> str:
    ...
    return cleaned or "Hello, how can I assist you?"
...
def main():
    ...
    print("=" * 60)
    print("Conversation Pipeline: Voice/Text → OpenRouter → Kokoro TTS")
    print("=" * 60)
    print("Make sure OPENROUTER_API_KEY is set and Kokoro-FastAPI is running!")
    ...
    while True:
        try:
            ...
            llama_response = sanitize_for_speech(query_llama(user_input))
            print(f"Assistant: {llama_response}\n")
